...
obj3d:GLUquadricObj;
  ...
  R_kv:array[1..NUM_OF_PARTICLES] of real;
...
for i:=1 to NUM_OF_PARTICLES-1 do begin
    for j:=(i+1) to NUM_OF_PARTICLES do begin
      if i<>j then  p_arr[i].R_kv[j] :=sqr(p_arr[i].x-p_arr[j].x)+sqr(p_arr[i].y-p_arr[j].y); //енверно считается расстояние
      p_arr[j].R_kv[i]:=p_arr[i].R_kv[j];
        //p_arr[i].R[]:=R[i,j];//можно ускорить если учесть что R[i,j]=R[j,i]
    end;
end;
for i:=1 to NUM_OF_PARTICLES do begin
    ...
    if i<>j then p_arr[i].Fx:=p_arr[i].Fx+(dF*(p_arr[j].x-p_arr[i].x)/(2*sqrt(p_arr[i].R_kv[j])*p_arr[i].R_kv[j]));
    if i<>j then p_arr[i].Fy:=p_arr[i].Fy+(dF*(p_arr[j].y-p_arr[i].y)/(2*sqrt(p_arr[i].R_kv[j])*p_arr[i].R_kv[j]));
...
p_arr[1].z:=0;
 randomize();
p_arr[2].x:=random(100);
p_arr[2].y:=random(100);
p_arr[2].x0:=1;
p_arr[2].y0:=1;
p_arr[2].z:=0;

p_arr[3].x:=random(100);
p_arr[3].y:=random(100);
p_arr[3].x0:=1;
p_arr[3].y0:=1;
p_arr[3].z:=0;}
...
fizika;
 for i:=1 to NUM_OF_PARTICLES do begin
    draw_obj(i,0.5,p_arr[i].x,p_arr[i].y,p_arr[i].z);
    end;
if NUM_OF_PARTICLES=5 then draw_obj(5,3,p_arr[5].x,p_arr[5].y,p_arr[5].z);
